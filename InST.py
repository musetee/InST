...
def main(prompt = '', content_dir = '', style_dir='',ddim_steps = 50,strength = 0.5, model = None, seed=42, device = 'cuda:1'):
    ddim_eta=0.0
    n_iter=1
    C=4
    f=8
                ...
                for n in trange(n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        if scale != 1.0:
                            ...
                            # ldm\models\diffusion\ddpm.py -> LatentDiffusion class line 605
                            # fixed after adding this line before
...
if __name__ == '__main__':
    ...
    # solve the bug of incompatible devices
    config.model.params.cond_stage_config.params.device=f"cuda:{device_id}"

    model = load_model_from_config(config, f"{ckpt}", verbose=True, device=device)
